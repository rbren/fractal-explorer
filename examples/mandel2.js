...
const getHSLForValue = function(val) {
	const h = val.toPolar().phi * 360 /  (2 * Math.PI);
	const s = .5; //Math.min(1, math.abs(val) / 100);
	const l = Math.abs(val) > MAX_ABS ? 0.0 : 0.5;
    return d3.hsl(h, s, l);
}
...
window.step = function(pixels, stepNumber) {
	...
	points.forEach(point => {
		//if (math.abs(point.z) > MAX_ABS) return;
	...
	points.forEach((point, idx) => {
		...
		const hsl = getHSLForValue(point.z);
